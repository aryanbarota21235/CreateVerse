...
export default function ServiceHeroButtons({ serviceName }: ServiceHeroButtonsProps) {
  ...
  return (
    <div className="mt-6 sm:mt-9 flex flex-row items-center gap-2 sm:gap-3 w-full sm:w-auto">
      <button
        type="button"
        onClick={() => openEnquiry(serviceName)}
        className="pressable group flex-1 sm:flex-initial inline-flex items-center justify-center gap-1.5 sm:gap-2 rounded-full bg-accent px-2.5 py-2.5 sm:px-8 sm:py-3.5 text-[11px] sm:text-sm font-bold text-white transition-all duration-150 hover:-translate-y-0.5 hover:bg-accent-dim shadow-md hover:shadow-lg hover:shadow-accent/25 cursor-pointer whitespace-nowrap"
      >
        <span className="hidden min-[380px]:inline">Get a Growth Strategy</span>
        <span className="min-[380px]:hidden">Growth Strategy</span>
