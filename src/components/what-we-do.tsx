        ...
        <Stagger className="mt-16 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {capabilities.map((c) => (
            <StaggerItem key={c.title}>
              <div className="group h-full rounded-2xl border border-paper-line bg-white p-8 shadow-card transition-all duration-300 hover:-translate-y-1 hover:shadow-lift">
                ...
                <p className="mt-3 text-sm leading-relaxed text-ink/55">{c.desc}</p>
              </div>
            </StaggerItem>
          ))}
          <StaggerItem>
            <div className="flex h-full items-center justify-center rounded-2xl bg-ink p-8 shadow-card">
              <p className="text-center font-display text-2xl font-semibold leading-snug tracking-tight text-white">
                One partner.<br />
