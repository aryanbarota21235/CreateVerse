...
export default function StatsStrip() {
  return (
    <section className="border-y border-paper-line/70 bg-white">
      <div className="container-site py-12 lg:py-16">
        <div className="grid grid-cols-2 gap-x-6 gap-y-10 lg:grid-cols-4">
          {stats.map((s, i) => (
            <Reveal key={s.label} delay={i * 0.08}>
              <div className="relative pl-0 lg:pl-6 lg:border-l lg:border-paper-line/80">
                <p className="font-display text-3xl font-bold tracking-tight text-ink sm:text-4xl lg:text-5xl">
                  {s.value}
                </p>
                <p className="mt-2 text-sm font-semibold text-ink/85">{s.label}</p>
                <p className="mt-1 text-xs text-ink/50">{s.note}</p>
              </div>
            </Reveal>
          ))}
        </div>
